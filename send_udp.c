...
int main(int argc, char *argv[]){
  ...
  struct hostent *hostptr; // creatig a pointer to a struct hostent named hostptr- this struct has info abut the host.
  struct { char head; u_long body; char tail; } msgbuf; // creatng an unnamed struct, the stucts var name is msgbuf it contains 2 chars and an unsigned long.
  ...
  hostptr = gethostbyname(argv[1]); // hostptr is the second value passed in the when running the code.
  ...
  msgbuf.body = htonl(getpid()); /* IMPORTANT! */ //the body of the message is the process id.
