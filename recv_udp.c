...
void printsin(struct sockaddr_in *s, char *str1, char *str2) {
  printf("%s\n", str1);
  printf("%s: ", str2);
  printf("--IP: %s ", inet_ntoa(s->sin_addr)); // printing the ip number
  printf("-- port: %d ", s->sin_port); // printing the port
  printf("\n");
}
...
int main(int argc, char *argv[]){
  ...
  struct { char head; u_long  body; char tail;} msg; // creating an unnamed struct, the stucts var name is msg it contains 2 chars and an unsigned long.
  ...
  bind(socket_fd, (struct sockaddr *)&s_in, sizeof(s_in)); // bindng the socket to the struct s_in 
